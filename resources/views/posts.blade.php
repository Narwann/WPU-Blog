...
@section('container')
    ...
    <div class="card mb-3">           
         @if ($posts[0]->image)
            ...
            @else
            <img src="https://source.unsplash.com/random/1200×400/?{{ $posts[0]->category->name }}" class="card-img-top" alt="{{ $posts[0]->category->name }}" class="img-fluid"> 
            @endif
           ...
           <h3 class="card-title"><a href="/posts/{{ $posts[0]->slug}}" class= "text-decoration-none text-dark">{{ $posts[0]->title }}</a></h3>
                    ...
                    <p> 
                        <small class="text-muted">
                            ...
                            class= "text-decoration-none" > {{ $posts[0]->author->name }}</a> in <a href="/posts?category={{ $posts[0]->category->slug }}" class= "text-decoration-none">{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }}</p>
                        </small>
                    </p>
     ...
     @foreach($posts->skip(1) as $post)
        <div class="col-md-4">
            <div class="card">
                ...
                @else
                <img src="https://source.unsplash.com/random/500×400/?{{ $post->category->name }}" class="card-img-top" alt="{{ $post->category->name }}" class="img-fluid">
                @endif
